...
import { digitalCardData ,downloadVCF } from "../data/jsautomotivedoncaster/jsautomotivedoncaster.js";
...
const iconMap = { Phone, Mail, Globe, MapPin, Facebook };

export default function Jsautomotivedoncaster() {
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        <Suspense fallback={<WebGLPlaceholder />}>
          <FloatingLines
            ...
            interactive={true}
            parallax={true}
            linesGradient={["#00D6AB", "#00D6AB", "#00D6AB", "#00D6AB", "#00D6AB"]}
          />
        </Suspense>
      </div>
      ...
      <div className="relative z-10 w-full flex flex-col items-center">
        ...
        <header className="w-full max-w-md px-4 pt-16 pb-6 flex flex-col items-center justify-center gap-4 text-center">
          <div className="relative shrink-0">
            <div className="absolute inset-0 rounded-3xl bg-[#00D6AB]/15 blur-2xl scale-110" />
            {logoFailed || !company.logo ? (
              <div className="relative flex w-20 h-20 md:w-28 md:h-28 items-center justify-center rounded-3xl border border-white/10 bg-slate-950 shadow-2xl">
                <div className="text-center leading-none">
                  <div className="text-2xl md:text-3xl font-black tracking-[0.18em] text-white">JS</div>
                  <div className="mt-1 text-[0.48rem] md:text-[0.58rem] font-semibold uppercase tracking-[0.2em] text-[#9FF5E4]">
                    Automotive
                  </div>
                </div>
              </div>
        ...
        <div className="w-full max-w-md px-4 pb-20">
                ...
                <li key={service.label}>
                  {service.href ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex text-center justify-center items-center rounded-lg border border-white/10 bg-white/5 px-4 py-3 transition hover:border-[#33E0BC]/40 hover:bg-white/10"
                    >
          ...
          {rating ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="block rounded-xl p-6 text-center mb-10 border border-white/10 bg-white/5 backdrop-blur-md transition hover:border-[#33E0BC]/40 hover:bg-white/8"
            >
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex justify-center items-center gap-2 text-white mb-10 hover:text-[#33E0BC] transition text-center"
          >
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/10">
              <MapPin className="w-4 h-4 text-[#33E0BC]" />
            </span>
            {location}
          </a>
              ...
              {socialLinks.map((social) => {
                ...
                return (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="h-12 w-12 rounded-full flex items-center justify-center transition bg-white/10 hover:bg-[#00D6AB] hover:text-black"
                  >
                    <IconComponent className="w-5 h-5" />
                  </a>
                );
          ...
          <section className="mb-10">
            <button
              onClick={downloadVCF}
              className="w-full h-14 flex items-center justify-center gap-2 rounded-lg font-semibold bg-[#00D6AB] text-black transition-all hover:bg-[#33E0BC]"
            >
          ...
          <footer className="text-center text-sm text-gray-400 opacity-60">
            <p>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-white transition hover:text-[#33E0BC] hover:underline"
              >
                Ansely
              </a>
            </p>
