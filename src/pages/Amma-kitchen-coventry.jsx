...
import { digitalCardData ,downloadVCF } from "../data/Amma-kitchen-coventry/amma.js";
      ...
      <div className="relative z-10 w-full flex flex-col items-center">
        ...
        <header className="w-full max-w-md px-4 pt-16 pb-6 flex flex-col items-center justify-center gap-4 text-center">
          <div className="relative shrink-0">
            <div className="absolute inset-0 rounded-3xl bg-amber-300/20 blur-2xl scale-110" />
            {logoFailed ? (
              <div className="relative flex w-20 h-20 md:w-28 md:h-28 items-center justify-center rounded-3xl border border-white/10 bg-slate-950 shadow-2xl">
                <div className="text-center leading-none">
                  <div className="text-2xl md:text-3xl font-black tracking-[0.18em] text-white">AMMA</div>
                  <div className="mt-1 text-[0.48rem] md:text-[0.58rem] font-semibold uppercase tracking-[0.28em] text-amber-200">
                    Kitchen
                  </div>
                </div>
              </div>
            ) : (
              <div className="relative flex w-22 h-22 md:w-30 md:h-30 items-center justify-center rounded-3xl bg-slate-950/95 p-2 shadow-2xl shadow-black/40 ring-1 ring-amber-200/20">
                <img
                  src={company.logo}
                  alt={company.name}
                  loading="lazy"
                  onError={() => setLogoFailed(true)}
                  className="h-full w-full object-contain scale-110 drop-shadow-[0_4px_18px_rgba(0,0,0,0.35)]"
                />
              </div>
            )}
          </div>
        ...
        <div className="w-full max-w-md px-4 pb-20">
          ...
          {rating ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="block rounded-xl p-6 text-center mb-10 border border-white/10 bg-white/5 backdrop-blur-md transition hover:border-[#63F080]/40 hover:bg-white/8"
            >
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex justify-center items-center gap-2 text-white mb-10 hover:text-[#63F080] transition text-center"
          >
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/10">
              <MapPin className="w-4 h-4 text-[#63F080]" />
            </span>
            {location}
          </a>
              ...
              {socialLinks.map((social) => {
                ...
                return (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="h-12 w-12 rounded-full flex items-center justify-center transition bg-white/10 hover:bg-[#45EC66]"
                  >
                    <IconComponent className="w-5 h-5" />
                  </a>
                );
          ...
          <section className="mb-10">
            <button
              onClick={downloadVCF}
              className="w-full h-14 flex items-center justify-center gap-2 rounded-lg font-semibold bg-[#45EC66] transition-all hover:bg-[#63F080]"
            >
          ...
          <footer className="text-center text-sm text-gray-400 opacity-60">
            <p>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-white transition hover:text-[#63F080] hover:underline"
              >
                Ansely
              </a>
            </p>
