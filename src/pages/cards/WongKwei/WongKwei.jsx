...
import { digitalCardDataWongKwei } from "../../../global";
import { downloadVCF } from "./WongKwei.js";
...
export default function WongKwei() {
    const {
        company,
        founder,
        contacts,
            ...
            <div className="relative z-10 w-full flex flex-col items-center">
                <header className="w-full max-w-md px-6 pt-16 pb-6 flex items-center justify-center text-center">
                    <div className="relative">
                        <div className="absolute inset-0 rounded-full bg-amber-400/15 blur-2xl scale-110" />
                            <div>
                                <div className="text-2xl font-black tracking-[0.16em] text-amber-50">WONG KWEI</div>
                                ...
                                <li key={service.label}>
                                    {service.href ? (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noreferrer"
                                            className="flex w-full items-center justify-center rounded-lg border border-white/10 bg-white/5 px-4 py-3 text-center text-white transition hover:border-amber-300/40 hover:bg-white/8"
                                        >
                                            {service.label}
                                        </a>
                                    ) : (
                                        <div className="flex w-full items-center justify-center rounded-lg border border-white/10 bg-white/5 px-4 py-3 text-center text-white">
                                            {service.label}
                                        </div>
                                    )}
                                </li>
